...
function fillTableComplete(data) {
    for (var i = 0; i < data.length; i++) {
        ...
        $('#table-complete').append('<tr><td>' + schedule + '</td><td>' + name + '</td><td>' + data[i].consultPatient.phone + '</td><td>' + moment(data[i].dateStart).add(1,'hours').format('DD/MM/YYYY HH:mm') + '</td><td>' + trainee + '</td><td>' + user + '</td></tr>');
    }
}
...
function fillTableConsultComplete(data) {
    for (var i = 0; i < data.length; i++) {
        ...
        $('#table-complete-all').append('<tr><td>' + schedule + '</td><td>' + name + '</td><td>' + data[i].consultPatient.phone + '</td><td>' + moment(data[i].dateStart).add(1,'hours').format('DD/MM/YYYY HH:mm') + '</td><td>' + trainee + '</td><td>' + user + '</td><td>' + status + '</td></tr>');
    }
}
...
$(document).ready(() => {
    ...
    $.ajax({
        ...
        success: (data) => {
            for (var i = 0; i < data.length; i++) {
                if (data[i].typeSchedule == 1) {
                    const name = truncar(data[i].consultPatient.name, 20)
                    $('#table-day').append('<tr><td>' + name + '</td><td>' + moment(data[i].dateStart).local().format('DD/MM/YYYY') + '</td><td>' + moment(data[i].dateStart).add(1,'hours').format("HH:mm") + '</td></tr>');
                }
            }
        }
    })
    ...
    $('#allConsultDays').click(() => {
        ...
        $.ajax({
            ...
            success: (data) => {
                for (var i = 0; i < data.length; i++) {
                    $('#table-day').append('<tr><td>' + data[i].consultPatient.name + '</td><td>' + moment(data[i].dateStart).format('DD/MM/YYYY') + '</td><td>' + moment(data[i].dateStart).add(1,'hours').format('HH:mm') + '</td></tr>');
                }
            }
        })
    })
